...
export default function Caption() {
  return (
    <View>
      <FlatList 
        data ={data}
        renderItem={({item})=> <View style={styles.FlexBox}> 
            <View style={styles.author}><Text style={styles.title}>{item.author}</Text></View> 
            <View style={{padding:10}}> 
              <Text>Born in : {item.country}</Text>
              <Text>Language : {item.language}</Text>
              <Text>One of book : {item.title}</Text>
              <Text>Written : {item.year}</Text>
            </View>
            <Text style={styles.link} onPress={() => Linking.openURL(item.link)} > Read on Wikipedia </Text>
          </View>}
